...
// Given an image file name, read in the data, try to decode it as an image,
// resize it to the requested size, and then scale the values as desired.
Status ReadTensorFromImageFile1(string file_name, const int input_height,
                               const int input_width, const float input_mean,
                               const float input_std,
                               std::vector<Tensor>* out_tensors) {
  auto root = tensorflow::Scope::NewRootScope();
...
Status PrintTopLabels(const std::vector<Tensor>& outputs,
  ...
  const int how_many_labels = 1;//std::min(5, static_cast<int>(label_count));
...
// Given the output of a model run, and the name of a file containing the labels
// this prints out the top five highest-scoring values.
Status PrintTopLabels1(const std::vector<Tensor>& outputs,
                      string labels_file_name) {
  std::vector<string> labels;
  size_t label_count;
  Status read_labels_status =
...
int main(int argc, char* argv[]) {
  ...
  int32 input_mean = 0;
  int32 input_std = 255;
  string input_layer = "image_batch";
  string output_layer = "top_k";
...
//  Tensor label(tensorflow::DT_INT64, TensorShape({1}));
//  auto label_vec = label.vec<long long>();
//  label_vec(0) = {31};
//  string label_layer = "label_batch";

  Tensor keep_prob(tensorflow::DT_FLOAT, TensorShape({1}));
  auto keep_prob_vec = keep_prob.vec<float>();
  keep_prob_vec(0) = {1};
  string keep_prob_layer = "keep_prob";

  Tensor train_flag(tensorflow::DT_BOOL, TensorShape({}));
  train_flag.scalar<bool>()()=false;
  string train_flag_layer = "train_flag";

  // Actually run the image through the model.
  std::vector<Tensor> outputs;
//  Status run_status = session->Run({{input_layer, resized_tensor}, {label_layer, label}, {keep_prob_layer, keep_prob}, {train_flag_layer, train_flag}},
//                                   {output_layer}, {}, &outputs);
  Status run_status = session->Run({{input_layer, resized_tensor}, {keep_prob_layer, keep_prob}, {train_flag_layer, train_flag}},
                                   { {output_layer + ":0", output_layer + ":1"}}, {}, &outputs);
  if (!run_status.ok()) {
    LOG(ERROR) << "Running model failed: " << run_status;
    return -1;
  }
  LOG(INFO) << keep_prob.DebugString();
//  // This is for automated testing to make sure we get the expected result with
//  // the default settings. We know that label 653 (military uniform) should be
//  // the top label for the Admiral Hopper image.
//  if (self_test) {
//    bool expected_matches;
//    Status check_status = CheckTopLabel(outputs, 31, &expected_matches);
//    if (!check_status.ok()) {
...
int main1(int argc, char* argv[]) {
  ...
  string image_path = tensorflow::io::JoinPath(root_dir, image);
  Status read_tensor_status =
      ReadTensorFromImageFile1(image_path, input_height, input_width, input_mean,
                              input_std, &resized_tensors);
  if (!read_tensor_status.ok()) {
    LOG(ERROR) << read_tensor_status;
    return -1;
